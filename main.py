...
def dashboard():
    ...
    try:
        ...
        cursor.execute("SELECT id_aluno, nome_aluno, ROUND(COALESCE(media_autocuidado,0),1) as media_autocuidado FROM db_funcae.vw_boletim_autocuidado;")
        dados_autocuidado = {r['id_aluno']: (r['nome_aluno'], float(r['media_autocuidado'] or 0.0)) for r in cursor.fetchall()}
        
        cursor.execute("SELECT id_aluno, nome_aluno, ROUND(COALESCE(media_linguagem,0),1) as media_linguagem FROM db_funcae.vw_boletim_linguagem;")
        dados_linguagem = {r['id_aluno']: (r['nome_aluno'], float(r['media_linguagem'] or 0.0)) for r in cursor.fetchall()}
        
        cursor.execute("SELECT id_aluno, nome_aluno, ROUND(COALESCE(media_socializacao,0),1) as media_socializacao FROM db_funcae.vw_boletim_socializacao;")
        dados_socializacao = {r['id_aluno']: (r['nome_aluno'], float(r['media_socializacao'] or 0.0)) for r in cursor.fetchall()}
        
        cursor.execute("SELECT id_aluno, nome_aluno, ROUND(COALESCE(media_motrocidade,0),1) as media_motrocidade FROM db_funcae.vw_boletim_motrocidade;")
        dados_motrocidade = {r['id_aluno']: (r['nome_aluno'], float(r['media_motrocidade'] or 0.0)) for r in cursor.fetchall()}
        
        cursor.execute("SELECT id_aluno, nome_aluno, ROUND(COALESCE(media_motrocidade_global,0),1) as media_motrocidade_global FROM db_funcae.vw_boletim_motrocidade_global;")
    ...
    try:
        if request.method == 'POST':
            ...
            if not all([tipo, nome_usuario, descricao]):
                ...
                try:
                    cursor.execute("""
                        INSERT INTO db_funcae.tbl_chamados_suport
        ...
        try:
            cursor.execute("SELECT * FROM db_funcae.tbl_chamados_suport ORDER BY data_criacao DESC")
